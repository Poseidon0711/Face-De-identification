...
loss = ['binary_crossentropy', contrastive_loss, categorical_crossentropy, 'mse']

loss_weights = [1, 1E5, 0.5, 2]
...
for epoch in range(0, nb_epoch):
	...
	for mini_batch_i in range(0, n_batches_per_epoch):
		...
		gen_loss = gan_nn.train_on_batch(X_images, 
		    ...
		    sim_labels[mini_batch_i*batch_size:(mini_batch_i+1)*batch_size], X_images])
		...
		progbar.add(1, values=[("Discr", D_log_loss),
		                                ("total", gan_loss),
		                                ("Gen", dis_loss),
		                                ("ver", ver_loss),
		                                ("sim", em_loss),
		...
		if ((epoch+1)%5 == 0):
			plot_generated_batch('ours',X_images, gen_nn, epoch, 'tng', mini_batch_i)
		batch_counter+=1
	...
	plt.legend()
	plt.savefig('/DATA/alakh/ours/ourgraph.png')


	gen_weights_path = '/DATA/alakh/ours/gen_weights2.h5'
	gen_nn.save_weights(gen_weights_path, overwrite=True)

	# disc_weights_path = '/DATA/alakh/ours/disc_weights2.h5'
	# dis_nn.save_weights(disc_weights_path, overwrite=True)

	# DCGAN_weights_path = '/DATA/alakh/ours/DCGAN_weights2.h5'
